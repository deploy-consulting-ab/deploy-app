...
import {
    addFieldPermissionToPermissionSetAction,
    removeFieldPermissionFromPermissionSetAction,
} from '@/actions/database/field-permission-actions';
import { toastRichSuccess } from '@/lib/toast-library';

export function PermissionSetFieldPermissions({ permissionSet, totalFieldPermissions }) {
    ...
    const handleFieldPermissionClick = (fieldPermissionId, isAssigned) => {
        ...
        startTransition(async () => {
            try {
                isAssigned
                    ? await removeFieldPermissionFromPermissionSetAction(
                          permissionSet.id,
                          fieldPermissionId
                      )
                    : await addFieldPermissionToPermissionSetAction(
                          permissionSet.id,
                          fieldPermissionId
                      );
