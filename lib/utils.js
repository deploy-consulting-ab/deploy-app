...
import { HOLIDAY_TYPE_ID, MONTHS, SWEDISH_BANK_HOLIDAYS } from '@/actions/flex/constants';
...
export const calculateHolidays = (absenceData) => {
    ...
    const HOLIDAY_FISCAL_YEAR_START = new Date(Date.UTC(currentYear, 3, 1)); // April 1st UTC
    const HOLIDAY_FISCAL_YEAR_END = new Date(Date.UTC(currentYear + 1, 2, 31)); // March 31st next year UTC
    ...
    console.log(chalk.green(), 'holidayEntries', holidayEntries);
    ...
    const holidayPeriods = holidayEntries.map((entry) => {
        ...
        // Adding 'Z' suffix ensures they're parsed as UTC
        const fromDate = new Date(entry.FromDate + 'Z');
        fromDate.setUTCHours(12, 0, 0, 0);
        ...
        while (currentDate <= toDate) {
            ...
            if (
                ...
                if (isCurrentFiscalYear(currentDate)) {
