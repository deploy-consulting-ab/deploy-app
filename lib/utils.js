 ...
 *   Weeks sorted newest first, as returned by getAssignmentTimereports.
...
export function buildMonthlyOccupancyFromWeeks(timereports, referenceDate) {
    ...
    return sortedKeys.map((key) => {
        ...
        const rate = totalMonthlyHours > 0 ? Math.round((externalHours / totalMonthlyHours) * 10000) / 100 : 0;
...
export function buildFullMonthlyOccupancy(timereports, referenceDate) {
    ...
    return sortedKeys.map((key) => {
        ...
        const rate = totalMonthlyHours > 0 ? Math.round((externalHours / totalMonthlyHours) * 100) : 0;
...
/**
 * Group weekly timereports (from getAssignmentTimereports) into monthly buckets.
