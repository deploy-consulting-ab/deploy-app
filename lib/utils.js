...
export const calculateHolidays = (absenceData) => {
  ...
  const FISCAL_YEAR_START = new Date(currentYear, 3, 1); // April 1st
  const FISCAL_YEAR_END = new Date(currentYear + 1, 2, 31); // March 31st next year

  /**
   * Subfunction to check if date is in current fiscal year
   */
  const isCurrentFiscalYear = (date) => {
    return date >= FISCAL_YEAR_START && date <= FISCAL_YEAR_END;
  };

  // Filter only holiday entries
  const holidayEntries = absenceData.filter(
    (entry) => entry.AbsenceTypeId === HOLIDAY_TYPE_ID
  );
  ...
  return {
    ...
    holidayPeriods: holidayPeriods.map((period) => ({
      ...
      isCurrentFiscalYear:
        period.fromDate >= FISCAL_YEAR_START &&
        period.toDate <= FISCAL_YEAR_END,
    })),
  };
};
...
export const getStageColor = (projectStatus) => {
