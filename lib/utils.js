 ...
 * @param {string} dateStr - YYYY-MM-DD (any day in the target month)
 * @returns {string} YYYY-MM-DD of the first day of that month
 */
export const getMonthStartDate = (dateStr) => {
    const d = new Date(dateStr + 'T00:00:00Z');
    return formatDateToISOString(new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), 1)));
};

/**
 * Get the first day of a month offset from a given date string.
 * @param {string} dateStr - YYYY-MM-DD (any day in the reference month)
 * @param {number} monthOffset - Number of months to shift (negative for previous)
 * @returns {string} YYYY-MM-DD of the first day of the target month
 */
export const getAdjacentMonthStartDate = (dateStr, monthOffset) => {
    const d = new Date(dateStr + 'T00:00:00Z');
    d.setUTCDate(1);
 ...
 * working hours (weekdays excluding Swedish bank holidays), and returns the same
 * shape as transformOccupancyData so the card component needs no changes.
 *
 ...
 * @param {Object} rawData - Raw occupancy data from getRecentOccupancyRate
 * @returns {Object|null} Transformed occupancy data for OccupancyRatesCard component
 */
export const transformOccupancyData = (rawData) => {
    if (!rawData) return null;
