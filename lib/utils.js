...
export const formatDateToSwedish = (dateString) => {
    // Parse YYYY-MM-DD format and return in Swedish format (YYYY-MM-DD)
    const [year, month, day] = dateString.split('-').map(Number);
    return `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
};

export const formatDateToEnUSWithOptions = (dateString) => {
...
export const populatePermissions = (entityPermissions, totalPermissions) => {
    const permissions = [];
